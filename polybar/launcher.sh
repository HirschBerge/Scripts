...
launch_bar() {
  MONITOR=$1 IF_ETH=${ETH} IF_WIFI=${WIFI} polybar "$2" -r -c ~/.config/polybar/config &
}
...
launch_secondary_bars() {
  ...
  for line in $(xrandr --listactivemonitors | sed '1d' | grep -v '\*'); do
    ...
    launch_bar "${output}" top-secondary -c ~/.config/polybar/config
    launch_bar "${output}" bottom-secondary -c ~/.config/polybar/config
  done
}
