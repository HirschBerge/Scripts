...
class MangaDex:
    ...
    def get_manga_chapters(
        self, mg: Manga, params: dict = None, includes: list = None
    ) -> List[Chapter]:
        ...
        return self._retrieve_pages(
            f"{self.api}/manga/{mg.id}/feed", Chapter, call_limit=100, params=params
        )
    ...
    def get_manga_covers(self, mg: Manga, params: dict = None) -> List[Cover]:
        ...
        params["manga[]"] = mg.id
        return self._retrieve_pages(
            f"{self.api}/cover", Cover, call_limit=100, params=params
        )
    ...
    def get_user_list(self, limit: int = 100) -> List[Manga]:
        ...
        if not self.login_success:
            raise NotLoggedInError
        return self._retrieve_pages(
            f"{self.api}/user/follows/manga", Manga, limit=limit, call_limit=100
        )
    ...
    def get_user_updates(self, limit: int = 100, params: dict = None) -> List[Chapter]:
        ...
        params = params or {}
        return self._retrieve_pages(
            f"{self.api}/user/follows/manga/feed",
            Chapter,
            call_limit=100,
            limit=limit,
            params=params,
    ...
    def transform_ids(self, obj: str, content: List[int]) -> Dict:
        ...
        if post.status_code == 200:
            resp = post.json()
            return {
                x["data"]["attributes"]["legacyId"]: x["data"]["attributes"]["newId"]
                for x in resp
            }
        else:
            raise APIError(post)
    ...
    def search(
        self, obj: str, params: dict, limit: int = 100
    ) -> List[Union[Manga, Chapter, Group, Author, Cover, User]]:
        """Searches an object."""
        m = SearchMapping(obj)
        return self._retrieve_pages(
            f"{self.api}{m.path}", m.object, limit=limit, call_limit=100, params=params
        )

    def _retrieve_pages(
        self,
        url: str,
        obj: Type[Union[Manga, Chapter, Group, Author, Cover]],
        limit: int = 0,
        call_limit: int = 500,
        params: dict = None,
    ) -> List[Union[Manga, Chapter, Group, Author, Cover]]:
        params = params or {}
        data = []
        offset = 0
        resp = None
        ...
        while remaining:
            p = {
                "limit": limit if limit <= call_limit and limit != 0 else call_limit,
                "offset": offset,
            }
            p = {**p, **params}
